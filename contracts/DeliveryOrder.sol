...
contract INSWContract {
    ...
    modifier onlyCO(uint256 _id) {
        require(msg.sender == requests[_id].cargoOwner,"Only CO can call this function.");
        _;
    }
    constructor() payable {
    ...
    // melihat semua request yang pernah dikirim oleh CO
    ...
    function getCORequestDO (uint256 _id) public view onlyCO(_id) returns(RequestDO memory) {
    ...
    // OLEH SHIPPING LINE
    // melihat semua request yang dikirimkan ke sebuah SL
    function getAllRequestsDO () public view returns (RequestDO[] memory) {
        RequestDO[] memory allRequestDOs = new RequestDO[](totalRequestDO);
        for (uint i = 0; i < totalRequestDO; i++){
            RequestDO storage item = requests[i];
            if(item.SL != msg.sender) {
                continue;
            }
            allRequestDOs[i] = item;
        }
